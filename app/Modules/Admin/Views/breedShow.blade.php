...
<div class="container pb-5">
 <div class="row py-3">
 	<h4>Породы</h4>
 	<div class="col-12">
 		<a href="{{ url('/admin/breeds') }}" class="btn btn-success"><i class="fas fa-arrow-left"></i></a>
 	</div>
 </div>
 <div class="row">	    
		<div class="card col-12" style="background-color: #DCDCDC;">
          <div class="container">
		      <div class="row">		  
			  	<div class="col-6">
			  		<img src="{{ asset('image/breeds').'/'.'my-dog-club-'.str_slug($item->title, '-').'-'.$item->img }}max.jpg" class="card-img-top mt-3" alt="*">			  		
			  	</div>
			  	<div class="col-6 bg-light d-flex justify-content-center flex-wrap" style="overflow-y: auto;max-height: 400px;">
			  		...
			  		<div class="border border-primary p-0 mb-1">
			  			<img src="{{ asset('image/breeds/'.$item->id).'/'.$img }}">
			  			<p><button class="btn btn-info" onclick="copyToClipboard('/image/breeds/{{ $item->id }}/{{ $img }}');">Копировать ссылку</button>
                                                   <button class="btn btn-danger" onclick="deleteImage('/image/breeds/{{ $item->id }}/{{ $img }}', this);">Удалить</button>
                                                </p>
			  		</div>
			  		@endforeach			  		
			  	</div>
			  	<div class="col-6 mt-3">
			  		<form action="/admin/breeds/upload" method="POST" enctype="multipart/form-data">
				        {{ csrf_field() }}
		  ...
		  <div class="card-body">            
		    ...
		    <form method="POST" action="/admin/breeds/update" id="breedsedit">
		    	{{ csrf_field() }}		    	
		    	<input type="hidden" name="id" value="{{ $item->id }}">		    	
		    	...
		    	<a href="{{ url('/admin/breeds/destroy', $item->id) }}" class="btn btn-danger"><i class="far fa-trash-alt"></i></a></div>          	
		    </form>
		  </div>
		</div>
	  </div>
...
$(document).ready(function() {
	var inputTitle = $('#breedsedit input[name = "title"]').val();	
	$.ajaxSetup({
      headers: {
       'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
               }
               });    
    $('#breedsedit input[name = "title"]').blur(function()
      {
      ...
      $.ajax({                
                  ...
                  url: '/ajax',                  
                  data: {						
					    procedure:'validBreedsTitle',
						title:$(this).val(),
						},
                  cache: false,              
                  success: function(result){
                   $('#title').text(result.message);
                   if (result.error == 1) {color = 'red';$('#breedsedit button').attr('disabled', true)}
                   if (result.error == 0) {color = 'green';$('#alias').val(result.alias);$('#breedsedit button').attr('disabled', false)}
